...
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: share of county pairs satisfying each proximity criterion, among the pairs selected by the TMO threshold and among all county pairs; computed from the pair-level file saved by {\tt savedyad}.  County centroids are taken from the 2018 Census cartographic boundary files bundled with this article's materials.\end{minipage}
\end{table}
...
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: ratios relative to the judicial-division-clustered standard error (0.045); brackets report the share of off-diagonal county pairs admitted to the variance.\end{minipage}
\end{table}
...
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: TMO applied to the Bernini et al.\ (2023) specification under alternative auxiliary collections.  The fourth row deliberately adds three Black-population-share measures to illustrate an implementation error that the diagnostics do not flag (see below).\end{minipage}
\end{table}
...
The Step 4 checks pass, with two features worth noting relative to the county-level case: the estimated degrees of freedom are $36$, above the recommended floor but modest, and, with only 175 locations, the threshold curve is visibly noisier at low thresholds, though stable around the optimum $\hat\delta^\star = 0.39$ (Figure \ref{fig:acediag}).  Where the available collection is thin, it can be supplemented from public sources, such as UN and World Bank series in this application.
...
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: share of country pairs satisfying each proximity criterion, among the 822 pairs selected by the TMO threshold ($\hat\delta^\star=0.39$) and among all country pairs.  Computed from the pair-level file saved by {\tt savedyad}; the accompanying code documents the construction.\end{minipage}
\end{table}

Table \ref{tab:acecompare} carries this to Step 6.  The Conley adjustment moves the standard error little (a ratio of 1.03): consistent with Step 5, the dependence in these data is not organised by distance.  TMO raises the standard error by 17\%, and adding the Conley disk to TMO changes essentially nothing, as the distance-based pairs contribute little beyond what the auxiliary outcomes identify.  In all cases the effect of democratisation remains statistically significant.
...
\toprule
 & SE & Ratio to clustered \\
\midrule
Cluster by country            & 0.230 & 1.00          \\
Conley (650 mi)               & 0.236 & 1.03 [2.9\%]  \\
TMO                           & 0.268 & 1.17 [5.4\%]  \\
TMO + Conley (650 mi)         & 0.268 & 1.17 [7.8\%]  \\
\bottomrule
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: ratios relative to the country-clustered standard error (0.230); brackets report the share of off-diagonal country pairs admitted to the variance.  SCPC rows are omitted because the current {\tt scpc} release does not support this clustered dynamic panel through {\tt scpc\_cmd()}.\end{minipage}
\end{table}

Finally, the Step 7 exercise in Table \ref{tab:aceylist} shows the consequences of a thin collection more sharply than the county-level case.  With a one-series-per-concept collection (18 outcomes), $\widehat{df}$ sits at the recommended floor of 20, the threshold rises to $0.50$, and the estimated ratio falls below one---an unstable, noise-dominated selection rather than evidence that clustering was conservative.  The full package collection (32 outcomes, $\widehat{df}=36$) delivers a stable 1.17.  The movement from $0.95$ to $1.17$ across nested, defensible collections is itself informative: an analysis that stopped at the thin list would have reported essentially no correction, reflecting lack of statistical power rather than absence of dependence.
...
\end{tabular}
\par\smallskip
\begin{minipage}{0.95\linewidth}\scriptsize Notes: TMO applied to the \citet{Acemogluetal2019} specification under alternative auxiliary collections.  $d$ is the number of outcomes used, $\widehat{df}$ the estimated degrees of freedom of the null, and the SE ratio is relative to the country-clustered standard error.  The third row deliberately adds seven alternative democracy measures to illustrate an implementation error that the diagnostics do not flag (see text).\end{minipage}
\end{table}
