...
The preferred specification (Table 2, Column 4 of Bernini et al.) regresses the 1964--1980 change in the share of Black elected officials on the interaction of the 1960 Black population share with federal coverage, together with the Black share, ten county controls and their coverage interactions, and state fixed effects, clustering by judicial division.  Estimating this specification yields $\widehat\theta = 0.103$, with a cluster-robust standard error of $0.045$.  The auxiliary collection should contain outcomes measured for the same locations, plausibly sharing the cross-location correlation structure of the main outcome, and not measuring the treatment itself.  Three exclusion rules are then applied: outcomes missing for more than half of the estimation sample are dropped; outcomes collinear with the model are dropped; and outcomes whose absolute correlation with the main outcome, the regressor of interest, or any primary control exceeds $0.8$ are dropped---we refer to this last rule as the \textit{correlation screen}.  Applying these rules to the package's variables yields $d=60$ auxiliary outcomes, including, for example, the change in log gubernatorial turnout between 1940 and 1960 and the change in NAACP branches between 1942 and 1964.  The log below runs {\tt tmo} augmenting the original judicial-division clustering:
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \par\smallskip
  \begin{minipage}{0.95\linewidth}\scriptsize Notes: produced by the {\tt plothist} and {\tt plotse} options; the panels correspond to Figure 5 of the companion paper.\end{minipage}
\end{figure}
...
\subsubsection{A second application: instrumental variables---Bazzi et al.\ (2023)}
\label{sscn:iv}
...
Applying the Step 2 rules to the package yields $d=85$ auxiliary outcomes---Republican vote shares in earlier elections, the Southern Black population share, and census, farm, and manufacturing outcomes, among others.  The IV setting adds one requirement: the exclusions must cover the instrument's ingredients as well as the regressor's.  Here the shift-share machinery---origin-state migrant stocks, predicted flows, and the associated truncation indicators---measures the treatment, and all of it is excluded by the same logic that excludes the regressor family.
...
Finally, the Step 7 exercise in Table \ref{tab:bazziylist} probes the collection choice in both directions.  Trimming the collection to $d=60$---by additionally excluding the secondary control sets of the paper's own Table 2 (the 1900 controls and sorting confounds) and the raw vote counts---drops the estimated degrees of freedom to $16$, below the recommended floor: the resulting ratio happens to be similar here, but the diagnostics correctly flag the trimmed collection as underpowered, and Section \ref{sscn:guide}'s advice applies---prefer the broader defensible collection.  The misuse row adds four measures from the regressor's family; as in the guide application, the reported diagnostics look unremarkable, which is precisely why treatment measures must be excluded by design rather than by inspection.
