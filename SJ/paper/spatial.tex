...
\subsubsection{Cross-country data: democracy and growth}
The procedures discussed in this paper are in no way specific to U.S.\ counties: any setting with a cross-section (or panel) of locations and a collection of auxiliary outcomes is amenable to TMO, including cross-country data.  Indeed, \citet{Dellavignaetal2025} apply the procedure to the country-level panel of \citet{Acemogluetal2019}, who estimate the effect of democratisation on GDP per capita across 175 countries, finding that the TMO adjustment increases the reported standard error by around 6\%\ when combined with the original clustering.  In this subsection we will replicate and extend this exercise using the {\tt tmo} command, using the collection of country-level covariates available in the replication materials as auxiliary outcomes.
% TODO [NP/DC]: on hold until we obtain the Acemoglu et al. (2019) replication
% package; then: (i) reproduce Table 2 Column 3 of the original paper, (ii) apply
% tmo augmenting country-level clustering, (iii) compare with Conley/SCPC
% combinations as in the county example.
