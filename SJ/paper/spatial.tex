...
\subsubsection{Reproducing the results of the companion paper: Bernini et al.\ (2023)}
\label{sscn:validation}
Before turning to guidance for applied researchers, we verify that the command reproduces the empirical results reported in the companion paper \citep{Dellavignaetal2025}.  We use the application whose inputs are fully public and self-contained: the re-analysis of Bernini et al.\ (2023), a study of the effect of the Voting Rights Act on Black office-holding across 971 Southern counties.  In that application, all 60 auxiliary outcomes are drawn from the original paper's replication package, so the exercise can be reproduced from publicly available materials alone.

The preferred specification (Table 2, Column 4 of Bernini et al.) regresses the 1964--1980 change in the share of Black elected officials on the interaction of the 1960 Black population share with federal coverage, together with the Black share, ten county controls and their coverage interactions, and state fixed effects, clustering by judicial division.  Estimating this specification yields $\widehat\theta = 0.103$ (cluster SE $0.045$), matching the published $0.10$ ($0.04$).  The auxiliary collection is constructed following the selection rules described in Appendix E.2 of the companion paper: all feasible package variables, excluding the outcome and regressor families, the primary controls and their interactions, outcomes missing for over half of the sample, and outcomes highly correlated with the outcome, the regressor, or any primary control.  Implementing the correlation screen at $|\rho|>0.8$ removes exactly seven variables and yields $d=60$, the number used in the companion paper.  The log below runs {\tt tmo} augmenting the original judicial-division clustering:

\begin{stlog}
\input{examples/berniniExample.tex.log}
\end{stlog}

Table \ref{tab:bernvalidation} places the command's output next to the published values.  The number of outcomes is identical, the share of cross-cluster pairs retained matches to the reported digit ($0.71$ versus $0.70$ percent), and the optimal threshold and degrees of freedom agree closely ($0.52$ versus $0.54$; $27.3$ versus $25.8$).  The diagnostic figures produced by {\tt plothist} and {\tt plotse}, generated by the accompanying code, correspond to Figure 5 of the companion paper.  One quantity is sensitive to implementation detail: the standard-error ratio is $1.15$ under our rule-based collection, against the published $1.37$, with alternative implementations of the same selection rules (varying the correlation screen and family exclusions) spanning $1.15$--$1.48$.  This reflects a documented property of the estimator rather than a discrepancy between implementations: Figure 6 of the companion paper shows the adjustment ranging over a comparable interval across subsets of these same 60 outcomes, and the exact variable list underlying the published figure is not itself part of the replication materials.  This experience motivates a recommendation we return to below: the auxiliary outcome list should be published verbatim alongside any TMO-adjusted result (the code accompanying this article does so), together with the adjustment's sensitivity across reasonable collections.
...
SE ratio (TMO/clustered)               & 1.15 [1.15--1.48] & 1.37 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: the bracketed range reports the ratio across alternative implementations of the outcome-selection rules described in Appendix E.2 of the companion paper, whose exact variable list is not part of the replication materials; Figure 6 of that paper documents comparable sensitivity across subsets of the same collection.}
\end{table}
...
\begin{enumerate}
  ...
  \item Inspect the two diagnostic figures against the criteria set out in Step 4 below; if either check fails, revisit the outcome collection before proceeding.
  \item Use the saved pair-level data to establish whether the detected dependence is organised by geography, which determines whether distance-based corrections suffice or TMO is required.
...
We illustrate each step using \citet{Acemogluetal2019}, who estimate the effect of democratisation on economic growth in a panel of 175 countries observed between 1960 and 2010, in the dynamic specification
\begin{equation*}
  \log y_{ct} = \beta\, dem_{ct} + \sum_{j=1}^{4}\gamma_j \log y_{c,t-j} + \alpha_c + \delta_t + \epsilon_{ct},
\end{equation*}
with standard errors clustered by country (their Table 2, Column 3, described by the authors as their preferred specification).

\paragraph{Steps 1--3: baseline, outcomes, and the augmented run.}
The baseline should first be reproduced with a supported command.  Here the original {\tt xtreg, fe} is replaced by {\tt areg}, the identical within estimator: this returns the published point estimate exactly, $\widehat\beta = 0.787$, with a cluster-robust standard error of $0.230$ ($0.226$ under {\tt xtreg}'s degrees-of-freedom convention).\footnote{Two practical notes for dynamic panels: lags should be generated as ordinary variables before calling {\tt tmo}, rather than written with time-series operators inside {\tt cmd()} (the command subsets internally to the estimation sample, and in an unbalanced panel time-series operators would silently re-compute lags across gaps), and the regressor of interest should be listed immediately after the dependent variable in {\tt cmd()}.}  The auxiliary collection should contain outcomes that are measured for the same locations and periods, plausibly share the cross-location correlation structure of the main outcome, and do not measure the treatment itself; series should be cleaned to a common scale before use (normalising size-dependent variables, taking logarithms of positive levels, standardising within period, and winsorising extreme tails), and outcomes that are highly correlated with the outcome, regressor, or controls, heavily missing, or collinear with the model should be excluded.  Applying these rules to the replication package yields roughly thirty country-year economic series---education attainment and enrolment, trade, investment, TFP, child mortality, tax revenues, market reforms, unrest, population structure, and external assets---while the package's alternative democracy indices are excluded because they measure the treatment.  The log below runs {\tt tmo} augmenting the original clustering, and requests the diagnostic output used in the following steps:
...
Two checks should be carried out before any TMO adjustment is used, based on the figures produced by {\tt plothist} and {\tt plotse} and displayed in Figure \ref{fig:acediag}.  In the correlation histogram (panel a), verify that: (i) the centre of the distribution is symmetric, unimodal, and closely tracked by the fitted Gaussian null---the central mass is dominated by null pairs, and a skewed or poorly-fitted centre indicates excessive common correlation across the auxiliary outcomes, invalidating the estimated null; (ii) there is visible excess mass in the tails relative to the fitted null---this excess is the evidence of non-null pairs that the procedure is designed to capture; and (iii) the implied degrees of freedom ($\widehat{df}=1/\hat{v}$, reported in the output) reach at least 20, with larger values advisable in panel settings.  In the present application the fit is adequate, the tail excess is clear, and $\widehat{df}=36$.  In the threshold plot (panel b), verify that: (i) the curve is smooth and forms a plateau around $\hat\delta^\star$, so that the reported adjustment does not depend on the exact threshold; (ii) the ratio tends to one as $\delta\to 1$, since with no pairs selected the baseline standard error must be recovered; and (iii) erratic movement is confined to low thresholds, where selection is dominated by noise pairs.  With 175 locations the left portion of the curve is noisier than in county-level applications; this is the small-sample behaviour documented in the companion paper, where with as few as 60 countries (in the re-analysis of Funke et al.\ 2023) the histogram centre departs visibly from the Gaussian, the threshold curve is erratic, and the procedure is unreliable.  Here the curve is stable around the optimum $\hat\delta^\star = 0.39$.  When either check fails, the appropriate response is a larger or more relevant collection of outcomes, not a different threshold.
...
The choice of auxiliary outcomes is the most consequential decision in the procedure, and errors are possible in both directions.  As documented in the companion paper's re-analysis of Bernini et al.\ (2023), the adjustment grows with the number of relevant outcomes: with few outcomes, correlations are noisily estimated, the null distribution widens, the optimal threshold rises, and true non-null pairs are missed.  At the same time, adding outcomes indiscriminately can reduce the quality of the adjustment: appending some sixty less-relevant external series to that application's collection diluted the estimated correction, because outcomes that do not share the correlation structure of the main outcome pull the correlation estimates toward zero.  More outcomes are therefore not automatically better.  The degrees-of-freedom statistic, which measures the effective number of independent outcomes rather than the raw count, is the relevant metric, together with the stability of the adjustment across reasonable collections.  Table \ref{tab:aceylist} reports this sensitivity exercise for the present application.  With a one-series-per-concept collection (18 outcomes), $\widehat{df}$ sits at the recommended floor of 20, the threshold rises to $0.50$, and the estimated ratio falls below one---an unstable, noise-dominated selection rather than evidence that clustering was conservative.  The full package collection (32 outcomes, $\widehat{df}=36$) delivers a stable 1.17, and the supplemented 79-outcome collection used in the companion paper ($\widehat{df}=79$) continues the pattern with a lower threshold and a ratio of 1.06.  The movement from $0.95$ to $1.17$ across nested, defensible collections is itself informative: an analysis that stopped at the thin list would have reported essentially no correction, reflecting lack of statistical power rather than absence of dependence.
