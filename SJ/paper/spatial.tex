...
\subsubsection{Validating the implementation: \citet{Dellavignaetal2025}'s application to Bernini et al.\ (2023)}
\label{sscn:validation}
Before turning to guidance on when the procedure should be used, we document that the command reproduces the empirical results of \citet{Dellavignaetal2025} in the one application of theirs whose inputs are fully public and self-contained: their re-analysis of Bernini et al.\ (2023), a study of the effect of the Voting Rights Act on Black office-holding across 971 Southern counties.  This application is the running example of \citeauthor{Dellavignaetal2025}'s practical recommendations, and---by their account---all 60 auxiliary outcomes are drawn from the paper's own replication package, so that everything required travels with the original paper.

We first reproduce the published estimate: the preferred specification (Table 2, Column 4 of the original paper) regresses the 1964--1980 change in the share of Black elected officials on the interaction of the 1960 Black population share with federal coverage, together with the Black share, ten county controls and their coverage interactions, and state fixed effects, clustering by judicial division.  Our estimate is $\widehat\theta = 0.103$ (cluster SE $0.045$), matching the published $0.10$ ($0.04$).  We then construct the auxiliary collection following the selection rules stated in \citet[Appendix E.2]{Dellavignaetal2025}: all feasible package variables, excluding the outcome and regressor families, the primary controls and their interactions, outcomes missing for over half of the sample, and outcomes highly correlated with the outcome, the regressor, or any primary control.  Implementing the correlation screen at $|\rho|>0.8$ removes exactly seven variables and delivers exactly their $d=60$.  The log below runs {\tt tmo} augmenting the original judicial-division clustering:

\begin{stlog}
\input{examples/berniniExample.tex.log}
\end{stlog}

Table \ref{tab:bernvalidation} places our output next to the published figures.  The machinery aligns throughout: the number of outcomes is exact, the share of cross-cluster pairs retained matches to the reported digit ($0.71$ versus $0.70$ percent), and the optimal threshold and degrees of freedom agree closely ($0.52$ versus $0.54$; $27.3$ versus $25.8$).  The diagnostic figures produced by {\tt plothist} and {\tt plotse} (not shown; they are generated by the accompanying code) correspond to Figure 5 of \citeauthor{Dellavignaetal2025}.  The one quantity that is materially sensitive is the standard-error ratio itself: our rule-based collection yields $1.15$, against the published $1.37$, and defensible alternative implementations of the same verbal rules (varying the correlation screen and family exclusions) span $1.15$--$1.48$.  This is not a deficiency of either implementation but a property of the estimator that \citeauthor{Dellavignaetal2025} themselves document in this very application: their Figure 6 shows the adjustment ranging over roughly this same interval across subsets of these same 60 outcomes.  Since the exact variable list is not published, the ratio cannot be pinned beyond this range---which is precisely why, in the guide that follows, we recommend that researchers publish their auxiliary outcome list verbatim (as the accompanying {\tt codeSj.do} does) and report the adjustment's sensitivity across reasonable collections rather than a single number.

\begin{table}[H]
\centering
\caption{Validation against \citet{Dellavignaetal2025}: Bernini et al.\ (2023)}
\label{tab:bernvalidation}
\begin{tabular}{lcc}
\toprule
 & {\tt tmo} (this paper) & \citet{Dellavignaetal2025} \\
\midrule
...
\end{tabular}
\par\smallskip
{\footnotesize Notes: the bracketed range reports the ratio across defensible implementations of the verbal outcome-selection rules in \citet[Appendix E.2]{Dellavignaetal2025}, whose exact variable list is not published; their Figure 6 documents comparable sensitivity across subsets of the same collection.}
\end{table}

\subsubsection{When should you use {\tt tmo}?  A diagnostic guide, applied to \citet{Acemogluetal2019}}
\label{sscn:guide}
The examples so far document \textit{how} to run the command.  The more important question for a practitioner is \textit{when}: given a particular dataset and regression, is TMO needed at all, and should it replace or complement a distance-based correction?  \citet{Dellavignaetal2025} propose a series of empirical diagnostics to answer precisely these questions, and in this subsection we walk through them, end to end, in a well-known cross-country application.  \citet{Acemogluetal2019} estimate the effect of democratisation on economic growth in a panel of 175 countries observed between 1960 and 2010, in the dynamic specification
\begin{equation*}
  \log y_{ct} = \beta\, dem_{ct} + \sum_{j=1}^{4}\gamma_j \log y_{c,t-j} + \alpha_c + \delta_t + \epsilon_{ct},
\end{equation*}
with standard errors clustered by country (their Table 2, Column 3, described by the authors as their preferred specification).  Reproducing this specification with {\tt areg}---the within estimator, identical to the original {\tt xtreg, fe}---returns the published point estimate exactly, $\widehat\beta = 0.787$, with a cluster-robust standard error of $0.230$ ($0.226$ under {\tt xtreg}'s degrees-of-freedom convention).\footnote{Two practical notes for dynamic panels: lags should be generated as ordinary variables before calling {\tt tmo}, rather than written with time-series operators inside {\tt cmd()} (the command subsets internally to the estimation sample, and in an unbalanced panel time-series operators would silently re-compute lags across gaps), and the regressor of interest should be listed immediately after the dependent variable in {\tt cmd()}.}  As auxiliary outcomes we assemble roughly thirty country-year economic series from the replication package---education attainment and enrolment, trade, investment, TFP, child mortality, tax revenues, market reforms, unrest, population structure, and external assets---excluding the package's alternative democracy indices, which measure the treatment rather than outcomes.  The log below runs {\tt tmo} augmenting the original clustering, and requests the two diagnostic figures discussed next:

\begin{stlog}
\input{examples/acemogluExample.tex.log}
\end{stlog}

\paragraph{Diagnostic 1: is there dependence for TMO to find, and can it be measured reliably?}
\citet{Dellavignaetal2025} recommend two checks before trusting any TMO adjustment, both produced by the command ({\tt plothist} and {\tt plotse}) and displayed in Figure \ref{fig:acediag}.  The figures are not decoration: each comes with a specific list of features to verify.  In the correlation histogram (panel a), check that (i) the \textit{centre} of the distribution is symmetric, unimodal, and closely tracked by the fitted Gaussian null---the central mass is dominated by null pairs, and a skewed or poorly-fitted centre indicates too much common correlation across the auxiliary outcomes, invalidating the estimated null; (ii) there is visible \textit{excess mass in the tails} relative to the fitted null---that excess \textit{is} the evidence of non-null pairs which TMO exists to capture, and without it there is nothing for the procedure to find; and (iii) the implied degrees of freedom ($\widehat{df}=1/\hat{v}$, reported in the output) reach at least 20, the minimum \citeauthor{Dellavignaetal2025} recommend, with more advisable in panel settings.  Here the fit is adequate, the tail excess is clear, and $\widehat{df}=36$.  In the threshold plot (panel b), check that (i) the curve is \textit{smooth and forms a plateau} around $\hat\delta^\star$, so that the reported adjustment does not hinge on the exact threshold; (ii) the ratio tends to one as $\delta\to 1$ (with no pairs selected the baseline standard error must be recovered---a mechanical sanity check); and (iii) erratic movement is confined to low thresholds, where selection is dominated by noise pairs.  With only 175 locations the left portion of our curve is visibly noisier than in county-level applications---exactly the small-$N$ caution raised by \citeauthor{Dellavignaetal2025}, who show that with as few as 60 countries (\textit{e.g.}, in their re-analysis of Funke et al.\ 2023) the histogram centre is visibly non-Gaussian, the threshold curve is erratic, and the procedure is unreliable---but it is stable around the optimum $\hat\delta^\star = 0.39$.  When either diagnostic fails, the remedy is a larger or more relevant collection of outcomes, not a different threshold.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \par\smallskip
  {\footnotesize Notes: produced by the {\tt plothist} and {\tt plotse} options.  The panels are the direct analogues of Figures E.7 and E.8 in \citet{Dellavignaetal2025} for this application, and display the same qualitative anatomy (Gaussian-tracked centre with tail excess; noisy left segment, plateau at the optimum, convergence to one).  The quantitative differences---$\widehat{df}=36$ and $\hat\delta^\star=0.39$ here, versus 79 and 0.27 in the published figures---are entirely attributable to the auxiliary collections (the package's 32 outcomes versus their externally supplemented 79), as documented in Table \ref{tab:aceylist}.}
\end{figure}

\paragraph{Diagnostic 2: is the dependence geographic?}
This is the question that decides between corrections.  Following Table 1 of \citet{Dellavignaetal2025}, Table \ref{tab:acepredictors} asks what the country pairs selected by TMO ($|\hat\rho_{ii^\prime}| \geq \hat\delta^\star$, some 800 pairs) have in common.  Geographic proximity is informative but startlingly incomplete: only 11\%\ of selected pairs lie within 650 miles of one another (the Conley bandwidth \citeauthor{Dellavignaetal2025} adopt at the country level).  Being in the same world region does far better (47\%), initial income proximity adds little, and even taking all three criteria together leaves \textit{half} of the selected pairs unexplained.  The flavour of the unexplained half is instructive: among the most strongly correlated country pairs sit Sweden--Finland (317 miles apart), but also the United States--United Kingdom ($\hat\rho = 0.83$ at 4,400 miles) and Indonesia--Colombia ($\hat\rho = 0.67$ at 11,800 miles)---pairs linked by common growth trajectories, trade exposure, or commodity dependence rather than by geography, and invisible to any distance kernel.
...
\end{tabular}
\par\smallskip
{\footnotesize Notes: share of country pairs satisfying each proximity criterion, among the 822 pairs selected by the TMO threshold ($\hat\delta^\star=0.39$) and among all country pairs.  Computed from the pair-level file saved by {\tt savedyad}.}
\end{table}

\paragraph{Diagnostic 3: how much does each correction move the standard error?}
Table \ref{tab:acecompare} completes the argument by comparing the corrections directly, alongside the corresponding estimates in \citet[Table 5]{Dellavignaetal2025}.  The Conley adjustment barely moves the standard error (a ratio of 1.03---identical, along with its share of admitted pairs, to the published figure, a useful validation of the implementation): consistent with Diagnostic 2, the dependence in these data is simply not organised by distance.  TMO, by contrast, raises the standard error by 17\%, and adding the Conley disk on top of TMO changes essentially nothing---the distance-based pairs contribute little that the auxiliary outcomes had not already found.  Our adjustment is larger than the published 1.06, which is based on a collection of 79 auxiliary outcomes (the package's series supplemented with UN and World Bank data); the contrast is a live illustration of \citeauthor{Dellavignaetal2025}'s recommendation that the collection of outcomes---their number \textit{and} their relevance---is the researcher's most consequential choice.  In all cases the effect of democratisation remains statistically significant.
...
\begin{tabular}{lccc}
\toprule
 & SE & Ratio to clustered & \citet{Dellavignaetal2025} \\
\midrule
...
\end{tabular}
\par\smallskip
{\footnotesize Notes: ratios relative to the country-clustered standard error (0.230); brackets report the share of off-diagonal country pairs admitted to the variance.  The final column reports the corresponding ratios in \citet[Table 5]{Dellavignaetal2025}, whose TMO figures use 79 auxiliary outcomes.  The SCPC rows are omitted for our implementation: the current {\tt scpc} release does not support this clustered dynamic panel through {\tt scpc\_cmd()}.}
\end{table}

\paragraph{Diagnostic 4: how sensitive is the adjustment to the outcome collection?}
The choice of auxiliary outcomes is the researcher's most consequential decision, and both directions of error are possible.  \citet{Dellavignaetal2025} show, in their re-analysis of Bernini et al.\ (2023), that the adjustment \textit{grows} with the number of relevant outcomes---with few outcomes, correlations are noisily estimated, the null distribution widens, the optimal threshold rises, and true non-null pairs are missed---but also that \textit{indiscriminately adding} outcomes can hurt: appending some sixty less-relevant external series to their collection \textit{diluted} the estimated adjustment, because outcomes that do not share the correlation structure of the main outcome drag the correlation estimates toward zero.  More is therefore not automatically better; the degrees-of-freedom statistic---which measures the \textit{effective} number of independent outcomes, not the raw count---is the metric to watch, together with the stability of the adjustment across reasonable collections.  Table \ref{tab:aceylist} conducts this sensitivity exercise in our application.  With a thin one-series-per-concept collection (18 outcomes), $\widehat{df}$ sits at the recommended floor of 20, the threshold jumps to $0.50$, and the ``adjustment'' lands \textit{below} one---an unstable, noise-dominated selection, not evidence that clustering was conservative.  The full package collection (32 outcomes, $\widehat{df}=36$) delivers the stable 1.17.  \citeauthor{Dellavignaetal2025}'s supplemented 79-outcome collection ($\widehat{df}=79$) continues the pattern: a lower threshold, a sharper selection, and a moderated ratio of 1.06.  The swing from $0.95$ to $1.17$ across nested, defensible collections is itself the diagnostic: had we stopped at the thin list, we would have reported essentially no correction, for lack of statistical power rather than lack of dependence.
...
\citet{Dellavignaetal2025}, supplemented & 79 & 79.1 & 0.27 & 1.06 \\
\bottomrule
\end{tabular}
\par\smallskip
...
\paragraph{When badly-implemented TMO misleads.}  We close by cataloguing the failure modes a user should anticipate---several of which, importantly, are \textit{not} detectable from the diagnostic figures, and must instead be ruled out by design.  First, \textbf{including measurements of the treatment among the auxiliary outcomes}.  The third row of Table \ref{tab:aceylist} adds the package's seven alternative democracy indices to the collection: every reported diagnostic \textit{improves} ($\widehat{df}$ rises to 56, the threshold falls), and the ratio looks unremarkable---yet the design is now invalid, since the ``dependence'' recovered from treatment-correlated outcomes partly reflects the spatial pattern of the treatment itself, biasing the adjustment in an unknown direction.  The lesson is that outcome selection is an identification choice, not a diagnostic one.  Second, \textbf{operating at the degrees-of-freedom floor}: as the first row shows, an underpowered collection produces thresholds and adjustments that swing widely---including below one---across small perturbations of the outcome list.  Researchers should report the sensitivity exercise of Table \ref{tab:aceylist}, not a single run.  Third, \textbf{too few locations}: with samples in the low hundreds the null approximation itself degrades, and below roughly a hundred locations (as in \citeauthor{Dellavignaetal2025}'s 60-country re-analysis of Funke et al.\ 2023) both diagnostic figures visibly fail.  Fourth, \textbf{unprepared outcome data}: the auxiliary series should be cleaned to a common scale before use---\citet[Appendix B.1]{Dellavignaetal2025} normalise size-dependent variables by population, take logarithms of positive levels, standardise within period, and winsorise extreme tails---since a single heavy-tailed outcome can otherwise dominate the correlation estimates.  Fifth, \textbf{pervasive spatial dependence}: TMO's sparsity assumption requires that most pairs be (nearly) uncorrelated; in data with strong global trends of the spatial-unit-root type, no threshold separates null from non-null pairs, the histogram centre fails the Gaussian check, and trend removal must precede any correction.

\paragraph{Taking stock.}  The pattern of diagnostics maps directly into advice.  When the correlation-distribution and threshold-stability checks pass, and Table \ref{tab:acepredictors}-style diagnostics show dependence concentrated at short distances, distance-based corrections are appropriate and TMO should confirm rather than change them.  When---as here---a substantial share of detected dependence lies beyond any plausible distance kernel, distance-based corrections understate uncertainty and TMO (alone or combined) is called for.  And when the diagnostics fail---few locations, a thin or irrelevant outcome collection, degrees of freedom at or below roughly 20, or instability of the adjustment across reasonable outcome sets---the honest conclusion is that TMO cannot be applied reliably, and enriching the auxiliary collection comes before any standard-error claim.
