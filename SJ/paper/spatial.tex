...
\begin{enumerate}
  \item Reproduce the baseline estimate of interest with a supported estimation command.
  \item Assemble and prepare a collection of auxiliary outcomes, applying the selection and cleaning rules described below.
  \item Run {\tt tmo}, passing the original (clustered) command line through {\tt cmd()} so that the existing inference is augmented rather than replaced, and requesting the diagnostic output ({\tt plothist}, {\tt plotse}, {\tt savedyad}).
  \item Inspect the two diagnostic figures against the criteria set out below; if either check fails, revisit the outcome collection before proceeding.
  \item Use the saved pair-level data to establish whether the detected dependence is organised by geography, which determines whether distance-based corrections suffice, TMO is required, or both should be combined.
  \item Compare the available corrections, combining them where both geographic and non-geographic dependence are present.
  \item Assess the sensitivity of the adjustment to the outcome collection, and report it---together with the outcome list itself---alongside the adjusted standard errors.
\end{enumerate}

We illustrate each step using the application whose inputs are fully public and self-contained: Bernini et al.\ (2023), a study of the effect of the Voting Rights Act on Black office-holding across 971 Southern counties, re-analysed in the companion paper \citep{Dellavignaetal2025}.  All 60 auxiliary outcomes used there are drawn from the original paper's replication package, so every step below can be reproduced from publicly available materials; this also allows us to verify, along the way, that the command reproduces the results reported in the companion paper.

\paragraph{Steps 1--3: baseline, outcomes, and the augmented run.}
The preferred specification (Table 2, Column 4 of Bernini et al.) regresses the 1964--1980 change in the share of Black elected officials on the interaction of the 1960 Black population share with federal coverage, together with the Black share, ten county controls and their coverage interactions, and state fixed effects, clustering by judicial division.  Estimating this specification with {\tt regress} (factor-variable syntax replaces the original {\tt xi:} prefix) yields $\widehat\theta = 0.103$ (cluster SE $0.045$), matching the published $0.10$ ($0.04$).  The auxiliary collection should contain outcomes measured for the same locations, plausibly sharing the cross-location correlation structure of the main outcome, and not measuring the treatment itself; series should be brought to a common scale before use, and outcomes that are highly correlated with the outcome, regressor, or controls, heavily missing, or collinear with the model should be excluded.  Implementing these rules (stated in Appendix E.2 of the companion paper) over the package's variables---with the correlation screen at $|\rho|>0.8$, which removes exactly seven variables---yields exactly the $d=60$ outcomes used in the companion paper, including, for example, the change in log gubernatorial turnout between 1940 and 1960 and the change in NAACP branches between 1942 and 1964.  The log below runs {\tt tmo} augmenting the original judicial-division clustering:
...
Two checks should be carried out before any TMO adjustment is used, based on the figures produced by {\tt plothist} and {\tt plotse} and displayed in Figure \ref{fig:berndiag}.  In the correlation histogram (panel a), verify that: (i) the centre of the distribution is symmetric, unimodal, and closely tracked by the fitted Gaussian null---the central mass is dominated by null pairs, and a skewed or poorly-fitted centre indicates excessive common correlation across the auxiliary outcomes, invalidating the estimated null; (ii) there is visible excess mass in the tails relative to the fitted null---this excess is the evidence of non-null pairs that the procedure is designed to capture; and (iii) the implied degrees of freedom ($\widehat{df}=1/\hat{v}$, reported in the output) reach at least 20, with larger values advisable in panel settings.  In the threshold plot (panel b), verify that: (i) the curve is smooth and forms a plateau around $\hat\delta^\star$, so that the reported adjustment does not depend on the exact threshold; (ii) the ratio tends to one as $\delta\to 1$, since with no pairs selected the baseline standard error must be recovered; and (iii) erratic movement is confined to low thresholds, where selection is dominated by noise pairs.  Both checks pass here: the fit is adequate with clear tail excess and $\widehat{df}=27$, above the recommended floor, and the curve is stable around $\hat\delta^\star=0.52$.  When either check fails, the appropriate response is a larger or more relevant collection of outcomes, not a different threshold.  Table \ref{tab:bernvalidation} confirms that the quantities produced at this stage line up with those reported in the companion paper: the number of outcomes is identical, the share of cross-cluster pairs retained matches to the reported digit, and the threshold and degrees of freedom agree closely.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \par\smallskip
  {\footnotesize Notes: produced by the {\tt plothist} and {\tt plotse} options; the panels correspond to Figure 5 of the companion paper.}
\end{figure}

\begin{table}[H]
\centering
\caption{Reproduction of the companion paper's results for Bernini et al.\ (2023)}
\label{tab:bernvalidation}
\begin{tabular}{lcc}
\toprule
 & {\tt tmo} (this article) & \citet{Dellavignaetal2025} \\
\midrule
Point estimate $\widehat\theta$        & 0.103  & 0.10 \\
Original (clustered) SE                & 0.045  & 0.04 \\
Locations $n$                          & 971    & 971  \\
Auxiliary outcomes $d$                 & 60     & 60   \\
Degrees of freedom $\widehat{df}$      & 27.3   & 25.8 \\
Optimal threshold $\hat\delta^\star$   & 0.52   & 0.54 \\
\%\ of cross-cluster pairs retained    & 0.71   & 0.70 \\
SE ratio (TMO/clustered)               & 1.15 [1.12--1.48] & 1.37 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: the bracketed range reports the ratio across alternative implementations of the outcome-selection rules described in Appendix E.2 of the companion paper, whose exact variable list is not part of the replication materials; see Step 7.}
\end{table}

\paragraph{Step 5: establish whether the dependence is geographic.}
This step determines the choice between corrections.  Using the pair-level file saved by {\tt savedyad}, merge in location characteristics and compute, among the pairs selected by the threshold, the share that are close in geographic distance and in other observable dimensions; comparison with the corresponding shares among all pairs indicates which dimensions organise the detected dependence.  Table \ref{tab:bernpredictors} reports this exercise, in the manner of Table 1 of the companion paper.  Geography is clearly at work: relative to all pairs, selected pairs are three times as likely to lie within 150 miles of one another and eight times as likely to share a judicial division.  It is equally clear that geography is insufficient: fewer than a third of selected pairs are within the 150-mile band, similarity in population, poverty, or urbanisation adds little, and roughly 43\%\ of the selected pairs are close on none of the criteria considered.  Both a distance-based correction and TMO therefore have a role in this application.
...
\end{tabular}
\par\smallskip
{\footnotesize Notes: share of county pairs satisfying each proximity criterion, among the pairs selected by the TMO threshold and among all county pairs; computed from the pair-level file saved by {\tt savedyad}.  County centroids are taken from the 2018 Census cartographic boundary files bundled with this article's materials.}
\end{table}

\paragraph{Step 6: compare and, where warranted, combine corrections.}
Table \ref{tab:berncompare} compares the corrections directly, alongside the corresponding estimates in Table 5 of the companion paper.  Consistent with Step 5, both channels matter here: the Conley correction alone raises the standard error by roughly 40\%, and TMO contributes further pairs beyond the distance band.  The distance-based quantities, which do not depend on the auxiliary collection, reproduce the published figures essentially exactly---the Conley ratio (1.41 versus 1.40) and the shares of pairs admitted (9.0 and 9.6 percent) in particular.  The TMO-dependent ratios differ through the outcome collection, as discussed next.  This application also illustrates the general recommendation: when both geographic and non-geographic dependence are present, the combined estimator provides robustness to both.
...
\begin{tabular}{lccc}
\toprule
 & SE & Ratio to clustered & Companion paper \\
\midrule
Cluster by judicial division  & 0.045 & 1.00          & 1.00 \\
Conley (150 mi)               & 0.063 & 1.41 [9.0\%]  & 1.40 [9.0\%] \\
TMO                           & 0.051 & 1.15 [0.7\%]  & 1.37 [0.7\%] \\
TMO + Conley (150 mi)         & 0.063 & 1.42 [9.6\%]  & 1.51 [9.6\%] \\
SCPC / TMO + SCPC             & \multicolumn{2}{c}{---}       & 2.02 / 2.26 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: ratios relative to the judicial-division-clustered standard error (0.045); brackets report the share of off-diagonal county pairs admitted to the variance.  The final column reports the corresponding ratios in Tables 4 and 5 of the companion paper.  SCPC rows are cited from the companion paper.}
\end{table}

\paragraph{Step 7: assess sensitivity to the outcome collection, and report it.}
The choice of auxiliary outcomes is the most consequential decision in the procedure, and errors are possible in both directions: with too few relevant outcomes, correlations are noisily estimated, the null distribution widens, and true non-null pairs are missed, while adding outcomes that do not share the correlation structure of the main outcome pulls the correlation estimates toward zero and dilutes the adjustment.  The degrees-of-freedom statistic, which measures the effective number of independent outcomes rather than the raw count, is the relevant metric, together with the stability of the adjustment across reasonable collections.  Table \ref{tab:bernylist} reports the sensitivity exercise for this application: across alternative defensible implementations of the same selection rules, the diagnostics are essentially unchanged while the ratio spans $1.12$--$1.48$, an interval that contains the published $1.37$ and is comparable to the variation that Figure 6 of the companion paper documents across subsets of these same outcomes.  Because the adjustment inherits this sensitivity, we recommend publishing the auxiliary outcome list verbatim alongside any TMO-adjusted result---the code accompanying this article does so---and reporting the adjustment's range across reasonable collections rather than a single number.
...
\midrule
Selection rules, correlation screen only          & 57 & 26.1 & 0.50 & 1.48 \\
Full selection rules (baseline)                   & 60 & 27.3 & 0.52 & 1.15 \\
Family exclusions only, no correlation screen     & 67 & 27.8 & 0.52 & 1.40 \\
Baseline $+$ regressor-family measures (misuse)   & 64 & 27.3 & 0.52 & 1.12 \\
Companion paper                                   & 60 & 25.8 & 0.54 & 1.37 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: TMO applied to the Bernini et al.\ (2023) specification under alternative auxiliary collections.  The fourth row deliberately adds three Black-population-share measures to illustrate an implementation error that the diagnostics do not flag (see below).  The final row reports the corresponding figures from the companion paper.}
\end{table}

\paragraph{Common implementation errors.}  Several implementation errors deserve attention; importantly, some are not detectable from the diagnostic figures and must instead be ruled out by design.  First, \textit{including measurements of the treatment among the auxiliary outcomes}.  The misuse row of Table \ref{tab:bernylist} adds three Black-population-share measures---the regressor's family---to the collection: the reported diagnostics are essentially unchanged and the ratio appears unremarkable, yet the design is invalid, since dependence recovered from treatment-correlated outcomes partly reflects the spatial pattern of the treatment itself, biasing the adjustment in an unknown direction.  Outcome selection is an identification choice, not a diagnostic one.  Second, \textit{operating at the degrees-of-freedom floor}: underpowered collections produce thresholds and adjustments that vary widely, including below one, across small changes to the outcome list (Section \ref{sscn:panel} provides an example); the sensitivity exercise of Table \ref{tab:bernylist} should be reported rather than a single run.  Third, \textit{too few locations}: with samples in the low hundreds the null approximation degrades, and below roughly one hundred locations both diagnostic figures fail visibly, as in the companion paper's 60-country re-analysis of Funke et al.\ (2023).  Fourth, \textit{unprepared outcome data}: auxiliary series should be brought to a common scale before use, following the protocol in Appendix B.1 of the companion paper (normalise size-dependent variables by population, take logarithms of positive levels, standardise within period, winsorise extreme tails), since a single heavy-tailed outcome can otherwise dominate the correlation estimates.  Fifth, \textit{pervasive spatial dependence}: the sparsity assumption requires that most pairs be approximately uncorrelated; in data with strong global trends of the spatial-unit-root type, no threshold separates null from non-null pairs, the histogram centre fails the Gaussian check, and trend removal must precede any correction.
...
\subsubsection{A second application: panel data---\citet{Acemogluetal2019}}
\label{sscn:panel}
A second application illustrates the workflow in a dynamic panel, and the behaviour of the procedure when the available outcome collection is thinner than one would like.  \citet{Acemogluetal2019} estimate the effect of democratisation on economic growth in a panel of 175 countries observed between 1960 and 2010, in the dynamic specification
\begin{equation*}
  \log y_{ct} = \beta\, dem_{ct} + \sum_{j=1}^{4}\gamma_j \log y_{c,t-j} + \alpha_c + \delta_t + \epsilon_{ct},
\end{equation*}
with standard errors clustered by country (their Table 2, Column 3, described by the authors as their preferred specification).  Reproducing this specification with {\tt areg}, the within estimator, returns the published point estimate exactly, $\widehat\beta = 0.787$, with a cluster-robust standard error of $0.230$ ($0.226$ under {\tt xtreg}'s degrees-of-freedom convention).\footnote{Two practical notes for dynamic panels: lags should be generated as ordinary variables before calling {\tt tmo}, rather than written with time-series operators inside {\tt cmd()} (the command subsets internally to the estimation sample, and in an unbalanced panel time-series operators would silently re-compute lags across gaps), and the regressor of interest should be listed immediately after the dependent variable in {\tt cmd()}.}  Applying the Step 2 rules to the replication package yields roughly thirty country-year economic series---education, trade, investment, TFP, mortality, taxes, reforms, unrest, population structure, and external assets---with the package's alternative democracy indices excluded because they measure the treatment.

\begin{stlog}
\input{examples/acemogluExample.tex.log}
\end{stlog}

The Step 4 checks pass, with two features worth noting relative to the county-level case: the estimated degrees of freedom are $36$---above the floor, but well below the $79$ obtained in the companion paper with a collection supplemented from UN and World Bank sources---and, with only 175 locations, the threshold curve is visibly noisier at low thresholds, though stable around the optimum $\hat\delta^\star = 0.39$ (Figure \ref{fig:acediag}).

\begin{figure}[H]
  \caption{Diagnostics for the application to \citet{Acemogluetal2019}}
  \label{fig:acediag}
  \begin{subfigure}{0.49\textwidth}
      \includegraphics[width=\linewidth]{figures/acemoglu_hist.png}
    \caption{Correlations and fitted null}
    \label{fig:acediag_hist}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.49\textwidth}
      \includegraphics[width=\linewidth]{figures/acemoglu_se_ratio_over_thres.pdf}
    \caption{SE ratio across thresholds}
    \label{fig:acediag_se}
  \end{subfigure}
  \par\smallskip
  {\footnotesize Notes: produced by the {\tt plothist} and {\tt plotse} options.  The panels correspond to Figures E.7 and E.8 of the companion paper for this application.  The quantitative differences---$\widehat{df}=36$ and $\hat\delta^\star=0.39$ here, versus 79 and 0.27 in the published figures---are attributable to the auxiliary collections used (the package's 32 outcomes versus the externally supplemented 79), as documented in Table \ref{tab:aceylist}.}
\end{figure}

The Step 5 exercise delivers a starkly different verdict from the county-level application.  Table \ref{tab:acepredictors} reports the predictors of the selected country pairs.  Geographic proximity is informative but far from complete: only 11\%\ of selected pairs lie within 650 miles of one another (the Conley bandwidth adopted at the country level in the companion paper).  Common world region performs better (47\%), initial income proximity adds little, and the three criteria together account for approximately half of the selected pairs.  Individual pairs illustrate the pattern: among the most strongly correlated country pairs are Sweden--Finland (317 miles apart), but also the United States--United Kingdom ($\hat\rho = 0.83$ at 4,400 miles) and Indonesia--Colombia ($\hat\rho = 0.67$ at 11,800 miles), pairs that no distance kernel would connect.

\begin{table}[H]
\centering
\caption{Predictors of highly-correlated country pairs, \citet{Acemogluetal2019} data}
\label{tab:acepredictors}
\begin{tabular}{lcc}
\toprule
 & Among pairs with $|\hat\rho_{ii^\prime}|\geq\hat\delta^\star$ & Among all pairs \\
\midrule
...
\end{tabular}
\par\smallskip
{\footnotesize Notes: share of country pairs satisfying each proximity criterion, among the 822 pairs selected by the TMO threshold ($\hat\delta^\star=0.39$) and among all country pairs.  Computed from the pair-level file saved by {\tt savedyad}; the accompanying code documents the construction.}
\end{table}

Table \ref{tab:acecompare} carries this to Step 6, comparing the corrections alongside the corresponding estimates in Table 5 of the companion paper.  The Conley adjustment moves the standard error little (a ratio of 1.03, identical---along with its share of admitted pairs---to the published figure): consistent with Step 5, the dependence in these data is not organised by distance.  TMO raises the standard error by 17\%, and adding the Conley disk to TMO changes essentially nothing, as the distance-based pairs contribute little beyond what the auxiliary outcomes identify.  The adjustment here is larger than the published 1.06, which is based on a collection of 79 auxiliary outcomes (the package's series supplemented with UN and World Bank data); the difference reflects the role of the outcome collection, taken up in Step 7.  In all cases the effect of democratisation remains statistically significant.

\begin{table}[H]
\centering
\caption{Method comparison, \citet{Acemogluetal2019} application}
\label{tab:acecompare}
\begin{tabular}{lccc}
\toprule
 & SE & Ratio to clustered & Companion paper \\
\midrule
...
SCPC / TMO + SCPC             & \multicolumn{2}{c}{---}       & 1.01 / 1.10 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: ratios relative to the country-clustered standard error (0.230); brackets report the share of off-diagonal country pairs admitted to the variance.  The final column reports the corresponding ratios in Table 5 of the companion paper, whose TMO figures use 79 auxiliary outcomes.  The SCPC rows are omitted for our implementation because the current {\tt scpc} release does not support this clustered dynamic panel through {\tt scpc\_cmd()}.}
\end{table}

Finally, the Step 7 exercise in Table \ref{tab:aceylist} shows the consequences of a thin collection more sharply than the county-level case.  With a one-series-per-concept collection (18 outcomes), $\widehat{df}$ sits at the recommended floor of 20, the threshold rises to $0.50$, and the estimated ratio falls below one---an unstable, noise-dominated selection rather than evidence that clustering was conservative.  The full package collection (32 outcomes, $\widehat{df}=36$) delivers a stable 1.17, and the supplemented 79-outcome collection used in the companion paper ($\widehat{df}=79$) continues the pattern with a lower threshold and a ratio of 1.06.  The movement from $0.95$ to $1.17$ across nested, defensible collections is itself informative: an analysis that stopped at the thin list would have reported essentially no correction, reflecting lack of statistical power rather than absence of dependence.
...
Companion paper, supplemented collection & 79 & 79.1 & 0.27 & 1.06 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Notes: TMO applied to the \citet{Acemogluetal2019} specification under alternative auxiliary collections.  $d$ is the number of outcomes used, $\widehat{df}$ the estimated degrees of freedom of the null, and the SE ratio is relative to the country-clustered standard error.  The third row deliberately adds seven alternative democracy measures to illustrate an implementation error that the diagnostics do not flag (see text).  The final row reports the corresponding figures from the companion paper.}
\end{table}
