...
\citet{Dellavignaetal2025} recommend two checks before trusting any TMO adjustment, both produced by the command ({\tt plothist} and {\tt plotse}) and displayed in Figure \ref{fig:acediag}.  First, the central mass of the distribution of pairwise correlations should be well approximated by the fitted Gaussian null: a heavily skewed centre indicates too much common correlation across the auxiliary outcomes, and an unreliable null.  Here the fit is adequate (panel a), with clear excess mass in the tails---the non-null pairs TMO exists to capture---and an estimated 36 degrees of freedom, above the minimum of 20 that \citeauthor{Dellavignaetal2025} recommend, though they caution that panel settings benefit from more.  Second, the standard error should vary smoothly with the threshold in the neighbourhood of $\hat\delta^\star$ (panel b).  With only 175 locations the curve is visibly noisier than in county-level applications---exactly the small-$N$ caution raised by \citeauthor{Dellavignaetal2025}, who show that with as few as 60 countries (\textit{e.g.}, in their re-analysis of Funke et al.\ 2023) both diagnostics fail and the procedure becomes unreliable---but it is stable around the optimum $\hat\delta^\star = 0.39$.  When either diagnostic fails, the remedy is a larger or more relevant collection of outcomes, not a different threshold.
