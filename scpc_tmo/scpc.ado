...
set matastrict on
mata mata clear // don't set this off, otherwise something wrong
mata 
...
{	
	...
	st_global("scpc_cv",strofreal(cv,"%50.20f")) // for tmo addition to store CV
}
...
{	
	...
	for(i=1;i<=cols(Wfin);i++){
		...
		stata("cap drop scpc_rx")
		stata("qui hashsort `e(clustvar)'") // added
