*! version 0.9.0b3-matacore 2026-07 -- prototype: cross-section fast path computes threshold+SE entirely in mata (no dyad dataset, no wide vectors). Program named tmo; never load together with any other tmo version (mata collisions). Validated identical SEs vs committed dev; ~37% faster; fixes stale-threshold and same_cl-missing reporting bugs.
